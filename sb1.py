...
def init_traj_diag13(ntime):
    ...
    # find number of existing test tractories: updates nprobt
        ...
        mdiag1.mfnptraj1(s1.ppart, s1.kpic, in1.nprobt)
...
def traj_diag13(ppart, kpic, partd, fvtp, fvmtp):
    ...
    mdiag1.mptraj1(ppart, kpic, s1.partt, tdiag)
    if ((in1.nst == 1) or (in1.nst == 2)):
...
def bread_restart13(iur):
    ...
    s1.i1[:] = numpy.fromfile(iur, int_type, 1);
    it = s1.i1[0]
...
def dwrite_restart13(iur):
    ...
    if (in1.nta > 0):
        ...
        if ((in1.nda == 2) or (in1.nda == 3)):
            ...
            # write out spectrum sizes and data
            if (ita > 0):
                s1.i4[0] = numpy.size(vpks, 0);
                s1.i4[1] = numpy.size(vpks, 1)
                s1.i4[2] = numpy.size(vpks, 2);
                s1.i4[3] = numpy.size(vpks, 3)
                s1.i4.tofile(iur)
                vpks.tofile(iur)
        else:
            it = 0
