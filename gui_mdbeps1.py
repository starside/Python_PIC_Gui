...
# allocates vpkwet, vwket
   if (in1.ntet > 0):
      ...
      pc.addGraph("FT TRANSVERSE E.F. Y -OMEGA VS MODE", "Transverse Electric Field/Transverse Z Electric Field, -Omega vs Mode")
...
# vx-vy, vx-vz or vy-vz
            ...
            for i in xrange(0,3):
               if ((nn % 2)==1):
                  a,b,c = numpy.shape(sb1.ppart)
                  phasearr = numpy.empty( (2,b,c), dtype=sb1.ppart.dtype )
                  phasearr[1,:,:] = sb1.ppart[min(i+3,4)-1,:,:]
                  phasearr[0,:,:] = sb1.ppart[max(i+1,2)-1,:,:]
                  pc.showPhase(phasearr, sb1.kpic, plottype="EPHASEV"+str(i))
                  graf1.dpmgrasp1(s1.ppart,s1.kpic,' ELECTRON',ntime,
...
# vx-vy, vx-vz or vy-vz
               ...
               for i in xrange(0,3):
                  if ((nn % 2)==1):
                     a,b,c = numpy.shape(sb1.pparti)
                     phasearr = numpy.empty( (2,b,c), dtype=sb1.pparti.dtype )
                     phasearr[1,:,:] = sb1.pparti[min(i+3,4)-1,:,:]
                     phasearr[0,:,:] = sb1.pparti[max(i+1,2)-1,:,:]
                     pc.showPhase(phasearr, sb1.kipic, plottype="IPHASEV"+str(i))
                     graf1.dpmgrasp1(s1.pparti,s1.kipic,' ION',ntime,
