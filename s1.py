...
def reorder_ions1(irc2):
    ...
    while (irc2[0] > 0):
        ...
        if (irc2[0] == 1):
            ...
            ihole = numpy.empty((2, ntmax + 1, mx1), int_type, 'F')
            irc2[:] = 0
            msort1.wmporder1(pparti, ppbuff, kipic, ncl, ihole, tsort, nx, mx,
                             False, irc2)
        # ppbuff overflow
        elif (irc2[0] == 2):
            npbmx = int((1.0 + in1.xtras) * irc2[1])
        ...
        # sanity check for ions
    if (in1.monitor > 0):
        mpush1.mcheck1(pparti, kipic, nx, mx, irc)
        if (irc[0] != 0): exit(1)
...
def init_traj_diag1(ntime):
    ...
    # find number of existing test tractories: updates nprobt
        ...
        mdiag1.mfnptraj1(ppart, kpic, in1.nprobt)
...
def dwrite_restart1(iur):
    ...
    if (in1.ntw > 0):
        ...
        if (itw > 0):
            it = numpy.size(wt, 1)
            i2[0] = numpy.size(wt, 0);
            i2[1] = numpy.size(wt, 1)
            i2.tofile(iur)
            wt[0:itw, :].tofile(iur)
    ...
    if (in1.ntp > 0):
        ...
        if ((in1.ndp == 2) or (in1.ndp == 3)):
            ...
            if (itp > 0):
                i3[0] = numpy.size(pks, 0);
                i3[1] = numpy.size(pks, 1)
    ...
    if (in1.movion == 1):
        ...
        if (in1.ntdi > 0):
            ...
            if ((in1.nddi == 2) or (in1.nddi == 3)):
                ...
                # write out spectrum sizes and data
                if (itdi > 0):
                    i3[0] = numpy.size(pksdi, 0);
                    i3[1] = numpy.size(pksdi, 1)
    ...
    if (in1.ntv > 0):
        ...
        # write out time history array sizes and data
        if (itv > 0):
            i3[0] = numpy.size(fvtm, 0);
            i3[1] = numpy.size(fvtm, 1)
    ...
    if (in1.ntt > 0):
        if ((in1.nst == 1) or (in1.nst == 2)):
            ...
            if (itt > 0):
                i3[0] = numpy.size(partd, 0);
                i3[1] = numpy.size(partd, 1)
